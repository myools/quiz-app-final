...
function generateStart(){
    $("header").append(startHtml[0].header);
    $("main").append(startHtml[0].main);
}
...
function generateComment(radioId, isCorrect){
    if (isCorrect === true){
        ...
        $("main").find(`label[for=${radioId}]`).after(`<p class='comment'>That is incorrect, the correct answer is ${questionData[questionArrayIndex].answers[questionData[questionArrayIndex].correctAnswer]}</p>`);
    }
}
...
function generateResults(){
    ...
    $("header").find("h2").before(`<h2 class="final-score">Your final score is ${score}/10 ${percent}%</h2>`);
}
...
function renderScore(){
    $("form").find("#correct-p").text(`Correct ${score}`);
    $("form").find("#incorrect-p").text(`Incorrect ${incorrect}`);
}
